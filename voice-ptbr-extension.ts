...
type WebviewInboundMessage =
	| { type: 'ready' }
	| { type: 'insertTranscript'; transcript: string }
	| { type: 'copyTranscript'; transcript: string }
...
function getOrCreatePanel(_extensionUri: vscode.Uri): vscode.WebviewPanel {
	if (currentPanel) {
		return currentPanel;
	}
	...
	currentPanel.webview.onDidReceiveMessage(async (message: WebviewInboundMessage) => {
		switch (message.type) {
			case 'ready':
...
async function revealAndConfigurePanel(panel: vscode.WebviewPanel): Promise<void> {
	panel.reveal(PANEL_COLUMN, true);
	await postSettingsToPanel(panel);
}

async function postSettingsToPanel(panel: vscode.WebviewPanel | undefined): Promise<void> {
	if (!panel) {
		return;
	}
...
async function insertTranscriptIntoChat(rawTranscript: string): Promise<void> {
...
async function closePanelAfterInsertIfNeeded(): Promise<void> {
	const settings = getSettings();
...
function getSettings(): VoiceSettings {
	...
	const locale = configuration.get<string>('locale', DEFAULT_LOCALE).trim() || DEFAULT_LOCALE;
	const sendImmediately = configuration.get<boolean>('sendImmediately', false);
	const keepPanelOpen = configuration.get<boolean>('keepPanelOpen', true);
